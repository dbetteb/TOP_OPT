...
def one_load_design_generator(nx = 100, ny = 100, volfraction = 0.4, nbr_loads = 2, rmin = 1.2, penalinit = 3.0, penalmed = 3.0, filt = 0, nu=0.3 ):
	...
	fixed_nodes =  np.arange(0, top.ny+1).tolist()
	...
	for load_node in opposite_side_nodes:
		nodes = [load_node]
		for teta in [90]: # [45,90,180,180+45,180+90] with teta = 180+90 only we have 596 output_plot 45,90,180,180+45,180+90
			if (len(set(nodes) - set(fixed_nodes)) == len(set(nodes))) : # i do not want to put load on a fixed edge
				...
				# Plot and Save only and only if the OBJECTIVE FUNCTION VALUE decreased more than 5 times i.e. the optimization problem converged
				if (top.comphist[0]/ top.comphist[-1] >= 5):
	...
	df.to_csv('./parameters/one_load_design_generation_'+datetime_info+'.csv')
...
def two_load_design_generator(nx = 100, ny = 100, volfraction = 0.4, nbr_loads = 2, rmin = 1.2, penalinit = 3.0, penalmed = 3.0, filt = 0, nu=0.3 ):
	...
	# opposite side nodes
	element = fixed_nodes[(len(fixed_nodes) - 1 )]
	starting_index = (possible_fixed_nodes.index(element)+top.nx)%len(possible_fixed_nodes) # using top.nx or top.ny is similar we consider having top.nx = top.ny
	ending_index = (possible_fixed_nodes.index(element)+2*top.nx)%len(possible_fixed_nodes)+1
	opposite_side_nodes = []
	if ending_index<starting_index:
	    opposite_side_nodes = possible_fixed_nodes[starting_index: ] + possible_fixed_nodes[0:ending_index]
	...
	for nodes in [random.sample(opposite_side_nodes,2) for _ in range(top.nx)]:
